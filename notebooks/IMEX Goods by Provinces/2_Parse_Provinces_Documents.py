...
# DBTITLE 1,Parse Provinces Documents - Overview
# MAGIC %md
# MAGIC # Step 2: Parse Provinces Customs Documents with AI
# MAGIC
# MAGIC This notebook uses `ai_parse_document()` to extract text and tables from Provinces PDF files (4 pattern) stored in Unity Catalog volumes.
# MAGIC
# MAGIC ## Process Flow
# MAGIC
# MAGIC 1. **Read PDFs from volumes**: Access files from Provinces-specific volumes:
# MAGIC    - `/Volumes/market_data/customs/provinces_import_goods/`
# MAGIC    - `/Volumes/market_data/customs/provinces_export_goods/`
...
# MAGIC 4. **Store raw results**: Save full JSON output to `market_data.customs.provinces_parsed_documents_raw`
# MAGIC 5. **Update tracking**: Mark parse status in `market_data.customs.provinces_document_processing_log`
# MAGIC
# MAGIC ## AI Parsing
# MAGIC
...
# MAGIC
# MAGIC ## Provincial Data Patterns
# MAGIC
# MAGIC * **Same parsing logic**: PDF structures are identical across FDI and Provinces reports
# MAGIC * **Different tables**: All data goes to `provinces_` prefixed tables
# MAGIC * **Same sub_category values**: "Import Goods" and "Export Goods"
# MAGIC * **URL patterns**: 4(vn-sb), 4(VN-DC), 4(VN-CT) for provincial statistics
# MAGIC
# MAGIC ## Output
# MAGIC
# MAGIC The raw table (`market_data.customs.provinces_parsed_documents_raw`) stores:
...
# DBTITLE 1,Parse Provinces Documents with ai_parse_document()
# One PDF per month carries both trade flows; the extraction step splits them.
for category in CATEGORIES:
    ...
    spark.sql(f"""
    MERGE INTO market_data.customs.provinces_parsed_documents_raw AS target
    USING (
      SELECT
        log.document_id,
        log.document_url,
...
print("\n" + "="*80)
print("Provinces Parsing workflow complete!")
print("="*80)