...
# MAGIC %md
# MAGIC # Update Provinces Customs URL Table
# MAGIC
...
TARGET_TABLE = "market_data.customs.provinces_customs_documents_url"
WORKFLOW_NAME = "IMEX Goods by Provinces"
PERIOD_TYPE = "MONTH"
...
REPORT_CONFIGS = [
    {
        "name": "Province IMEX Monthly",
        "title_patterns": [r"^xuat khau, nhap khau chia theo tinh/ ?thanh ?pho\s*-?\s*thang\s+\d{1,2}/20\d{2}$"],
        "url_patterns": [r"-4\(VN-", r"-4\(vn-"],
        # One PDF carries BOTH flows (export and import columns side by side), so
        # it is registered once. Splitting it per flow would download and
        # ai_parse_document the same file twice.
        "sub_categories": ["Provinces IMEX"],
    },
]
...
def discover_candidates(records: List[Dict]) -> List[Dict]:
    ...
    for record in records:
        ...
        for config in REPORT_CONFIGS:
            ...
            if not period:
                print(f"WARNING: Could not parse period, skipping: Update Provinces Customs URL Table -> {chosen_url}")
                continue
