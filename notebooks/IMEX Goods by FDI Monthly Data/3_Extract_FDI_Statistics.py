...
# DBTITLE 1,Extract FDI Statistics - Overview
# MAGIC %md
# MAGIC # Step 3: Extract Structured FDI Trade Statistics
# MAGIC
# MAGIC This notebook transforms raw parsed JSON from **FDI Import/Export goods** (3N/3X pattern) into structured trade statistics with proper hierarchy and data types.
# MAGIC
# MAGIC ## Process Flow
# MAGIC
# MAGIC 1. **Read raw parsed data**: Load JSON from `market_data.customs.fdi_parsed_documents_raw`
...
# MAGIC 6. **Insert to target**: Write to `market_data.customs.fdi_trade_statistics`
# MAGIC 7. **Update tracking**: Mark extraction status in `market_data.customs.fdi_document_processing_log`
# MAGIC
...
# MAGIC
# MAGIC * **Same extraction logic**: PDF structures and parsing are identical
# MAGIC * **Different tables**: All data goes to `fdi_` prefixed tables
# MAGIC * **Same schema**: Column structure matches the original
# MAGIC
# MAGIC ## Schema Output
# MAGIC
# MAGIC Key columns in `fdi_trade_statistics` table:
...
# DBTITLE 1,Extract and Transform FDI Statistics
import json
import re
...
print("="*80)
print("FDI STATISTICS EXTRACTION WORKFLOW")
print("="*80)
...
def extract_report_metadata(parsed_text):
    ...
    try:
        # Try monthly format: "Tháng 9 năm 2024"
...
# Step 1: Load raw parsed data from FDI tables
...
raw_df = spark.sql("""
    SELECT
        ...
        raw.parsed_json,
        log.parse_status
    FROM market_data.customs.fdi_parsed_documents_raw AS raw
...
# Step 3: Insert into fdi_trade_statistics table
if len(all_statistics) > 0:
    print("\nStep 3: Inserting into fdi_trade_statistics table...")
    print("-" * 80)
    ...
    spark.sql("""
        MERGE INTO market_data.customs.fdi_trade_statistics AS target
...
print("\n" + "="*80)
print("FDI Extraction workflow complete!")
print("="*80)