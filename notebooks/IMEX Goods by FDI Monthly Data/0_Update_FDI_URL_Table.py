...
# MAGIC %md
# MAGIC # Update FDI Customs URL Table
# MAGIC
...
TARGET_TABLE = "market_data.customs.fdi_customs_documents_url"
WORKFLOW_NAME = "IMEX Goods by FDI Monthly Data"
PERIOD_TYPE = "MONTH"
...
REPORT_CONFIGS = [
    {
        "name": "FDI Import Monthly",
        "title_patterns": [r"^nhap khau hang hoa cua doanh nghiep co von dau tu truc tiep nuoc ngoai \(fdi\) thang\s+\d{1,2}/20\d{2}$"],
        "url_patterns": [r"3N\(VN-", r"3n\(vn-"],
        "sub_category": "Import Goods",
    },
    {
        "name": "FDI Export Monthly",
        "title_patterns": [r"^xuat khau hang hoa cua doanh nghiep co von dau tu truc tiep nuoc ngoai \(fdi\) thang\s+\d{1,2}/20\d{2}$"],
        "url_patterns": [r"3X\(VN-", r"3x\(vn-"],
...
def remove_accents(text: str) -> str:
    if text is None:
        return ""
    # Handle Vietnamese đ/Đ specifically before NFD normalization
...
def discover_candidates(records: List[Dict]) -> List[Dict]:
    ...
    for record in records:
        ...
        for config in REPORT_CONFIGS:
            ...
            if not period:
                print(f"WARNING: Could not parse period, skipping: Update FDI Customs URL Table -> {chosen_url}")
                continue
