...
# MAGIC %md
# MAGIC # Update Countries Customs URL Table
# MAGIC
...
# MAGIC - Insert only missing `report_month | sub_category` combinations.
# MAGIC - Keep schema minimal: `url`, `sub_category`, `created_at`, `report_month`.
...
TARGET_TABLE = "market_data.customs.countries_customs_documents_url"
WORKFLOW_NAME = "IMEX Goods by Countries Monthly Data"
PERIOD_TYPE = "MONTH"
...
REPORT_CONFIGS = [
    {
        "name": "Countries Import by Origin Monthly",
        "title_patterns": [r"^nhap khau hang hoa tu mot so nuoc/vung lanh tho chia theo mat hang chu yeu thang\s+\d{1,2}/20\d{2}$"],
        "url_patterns": [r"5N\(VN-", r"5n\(vn-"],
        "sub_category": "Import by Origin",
    },
    {
        "name": "Countries Export by Destination Monthly",
        "title_patterns": [r"^xuat khau hang hoa sang mot so nuoc/vung lanh tho chia theo mat hang chu yeu thang\s+\d{1,2}/20\d{2}$"],
        "url_patterns": [r"5X\(VN-", r"5x\(vn-"],
        "sub_category": "Export by Destination",
    },
]
...
def discover_candidates(records: List[Dict]) -> List[Dict]:
    ...
    for record in records:
        ...
        for config in REPORT_CONFIGS:
            ...
            if not period:
                print(f"WARNING: Could not parse period, skipping: Update Countries Customs URL Table -> {chosen_url}")
                continue

            # One report can intentionally feed multiple downstream categories (e.g. provinces monthly).
...
# DBTITLE 1,Schema Migration: Ensure report_month Exists
...
def ensure_report_month_column(table_name: str):
    ...
    if "report_month" in df.columns:
        print("report_month column already exists.")
        return

    print(f"Adding report_month column to {table_name}...")
    try:
        spark.sql(f"ALTER TABLE {table_name} ADD COLUMNS (report_month STRING)")
        print("Added report_month column via ALTER TABLE.")
    except Exception as exc:
    ...
    for row in rows:
        row_dict = row.asDict()
        report_month = row_dict.get("report_month") or infer_period_from_existing_url(row_dict.get("url") or "")
        output_rows.append((
            row_dict.get("url"),
            row_dict.get("sub_category"),
            row_dict.get("created_at"),
    ...
    spark.createDataFrame(output_rows, schema).write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(table_name)
    print("Backfilled report_month for existing rows.")

ensure_report_month_column(TARGET_TABLE)
...
def existing_keys(table_name: str) -> set:
    ...
    if "report_month" not in df.columns:
        return set()
    keys = set()
    for row in df.select("report_month", "sub_category").where(F.col("report_month").isNotNull()).distinct().collect():
        keys.add(f"{row['report_month']}|{row['sub_category']}")
    return keys
...
for item in candidates:
    key = f"{item['report_month']}|{item['sub_category']}"
    if item["report_month"] == "Unknown":
        continue
...
if not rows_to_insert:
    ...
    now = datetime.utcnow()
    insert_rows = [(r["url"], r["sub_category"], now, r["report_month"]) for r in rows_to_insert]
    schema = StructType([
        StructField("url", StringType(), False),
        StructField("sub_category", StringType(), False),
        StructField("created_at", TimestampType(), True),
...
if rows_to_insert:
    ...
    for row in rows_to_insert[:5]:
        print(f"  - {row['report_month']} | {row['sub_category']} | {row['url']}")
